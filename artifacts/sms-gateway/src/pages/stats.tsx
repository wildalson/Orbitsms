...
export default function StatsPage() {
  ...
  const trafficByProduct = (() => {
    ...
    for (const t of traffic) {
      totals[t.productId] = (totals[t.productId] ?? 0) + t.count;
    }
  ...
  const dailyData = (() => {
    ...
    for (const t of traffic) {
      dayMap[t.day] = (dayMap[t.day] ?? 0) + t.count;
    }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Delivery performance and traffic analytics</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-card-border rounded-lg p-5">
          <h3 className="text-sm font-semibold text-foreground mb-4">Daily SMS Volume (This Month)</h3>
          {dailyData.length > 0 ? (
            ...
            <div className="h-[240px] flex items-center justify-center text-muted-foreground text-sm">No traffic data</div>
          )}
        </div>

        {/* Product distribution pie chart */}
        <div className="bg-card border border-card-border rounded-lg p-5">
          <h3 className="text-sm font-semibold text-foreground mb-4">Traffic by Product</h3>
          {trafficByProduct.length > 0 ? (
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie
                  data={trafficByProduct}
                  cx="50%"
                  cy="50%"
                  innerRadius={60}
                  outerRadius={90}
                  ...
                  dataKey="value"
                >
                  {trafficByProduct.map((entry, index) => (
      ...
      {stats && (
        <div className="bg-card border border-card-border rounded-lg p-5">
          ...
          <div className="flex items-end gap-3 h-32">
            {[
              { label: "Sent", count: stats.totalSent, color: "bg-primary" },
              { label: "Delivered", count: stats.totalDelivered, color: "bg-green-500" },
              { label: "Failed", count: stats.totalFailed, color: "bg-destructive" },
            ].map((item) => {
