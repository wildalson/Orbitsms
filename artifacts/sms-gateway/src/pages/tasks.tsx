  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-foreground">{t("smsTasks")}</h1>
          <p className="text-xs text-muted-foreground mt-0.5">{t("smsTasksSub")}</p>
        </div>
        <Link href="/tasks/new">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                ...
                <th className="text-left px-4 py-3 text-muted-foreground font-medium">{t("taskName")}</th>
                ...
                <th className="text-right px-4 py-3 text-muted-foreground font-medium">{t("total")}</th>
                <th className="text-right px-4 py-3 text-muted-foreground font-medium">{t("sent")}</th>
                <th className="text-right px-4 py-3 text-muted-foreground font-medium">{t("delivered")}</th>
                <th className="text-right px-4 py-3 text-muted-foreground font-medium">{t("failed")}</th>
                <th className="text-right px-4 py-3 text-muted-foreground font-medium">{t("costPhp")}</th>
                <th className="text-right px-4 py-3 text-muted-foreground font-medium">{t("created")}</th>
                <th className="text-center px-4 py-3 text-muted-foreground font-medium">{t("actions")}</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={11} className="px-4 py-12 text-center text-muted-foreground">
                    {t("noTasks")} <Link href="/tasks/new"><span className="text-primary cursor-pointer">{t("createOne")}</span></Link>
                  </td>
                </tr>
              ) : (
                data?.data.map((task) => (
                  <tr key={task.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-1.5 py-0.5 rounded border ${STATUS_STYLES[task.status]}`}>
        ...
        {data && data.total > pageSize && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-border">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="p-1.5 rounded hover:bg-muted/60 disabled:opacity-40 disabled:cursor-not-allowed text-muted-foreground">
                <ChevronLeft className="w-3.5 h-3.5" />
              </button>
              <span className="text-xs text-muted-foreground px-2">{page} / {totalPages}</span>
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="p-1.5 rounded hover:bg-muted/60 disabled:opacity-40 disabled:cursor-not-allowed text-muted-foreground">
