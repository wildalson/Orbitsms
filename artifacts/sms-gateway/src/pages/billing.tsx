      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            { label: "Failed", value: summary.totalFailed.toLocaleString(), color: "text-destructive" },
            { label: "Delivery Rate", value: `${summary.deliveryRate}%`, color: "text-amber-400" },
          ].map((s) => (
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-4 py-12 text-center text-muted-foreground">No billing records found</td>
                </tr>
              ) : (
                data?.data.map((b) => (
